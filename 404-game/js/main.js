// Remove game cover - add class to game wrapper
document.addEventListener('DOMContentLoaded', () => {
	var element = document.getElementById('game-wrap');
	var trigger = document.getElementById('game-cover');
	trigger.addEventListener('click', () => {
		element.classList.add('game-play');
	});
});
...
LoadingState.preload = function () {
    ...
    this.game.load.json('level:5', 'data/level06.json');
    this.game.load.json('level:6', 'data/level07.json');
    ...
    this.game.load.spritesheet('bg', 'images/bg_art.png', 960, 600);
    this.game.load.spritesheet('hero', 'images/hero3.png', 36, 42); 
...
PlayState._loadLevel = function (data) {
    ...
    this.bgArt = this.game.add.group();
    ...
    // spawn background art
    data.background.forEach(function (deco) {
        this.bgArt.add(
            this.game.add.image(deco.x, deco.y, 'bg', deco.frame));
    }, this);
    ...
    // enable gravity
    // const GRAVITY = 1200;
    this.game.physics.arcade.gravity.y = (this.game, data.gravity);
};
...
PlayState._spawnCoin = function (coin) {
    ...
    // animations
    sprite.animations.add('rotate', [0, 1, 2, 3, 4, 5], 16, true); // 6fps, looped
